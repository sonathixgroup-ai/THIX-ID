  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final mine = jobs
          .where(
            (j) =>
                (j.recruiterUserId ?? '').trim() == uid,
          )
          .map((j) => j.toMap())
          .toList();

      final apps =
          await _service.listRecruiterApplications(
        recruiterUserId: uid,
      );
      ...
      debugPrint(
        'RecruiterPortalPage.load failed err=$e',
      );
    ...
    return Scaffold(
              ...
              child: Column(
                children: [
                  ...
                  FutureBuilder<String>(
                    future:
                        AdminRbacService().fetchMyRole(),
                    builder: (context, snap) {
                      final role =
                          (snap.data ?? '').toString();

                      final canRecruit =
                          AdminRbacService.canAccess(
                                role: role,
                                minLevel: 2,
                              ) &&
                              role
                        ...
                        return Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  'Accès recruteur requis (role=recruiter).',
                                  style: context
                                      .textStyles
                                      .bodyMedium
                                      ?.copyWith(
      ...
      floatingActionButton: _tab == 0
          ? FloatingActionButton.extended(
              onPressed: () {
                context.go(
                  '${AppRoutes.admin}/jobs',
                );
              },
              backgroundColor:
                  LearningCyberColors.electricBlue,
      ...
      return Center(
        child: Text(
          ...
          style: context.textStyles.bodyLarge
              ?.copyWith(
            color: LearningCyberColors.textDim,
          ),
        ),
      );
    }
...
class _RecruiterApplicationsTab
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: LearningCyberColors.stroke
                .withOpacity(0.85),
          ),
        ),
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient:
                    LearningCyberGradients.glowBlue(),
              ),
...
class _JobsBackground extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      decoration: BoxDecoration(
        gradient:
            LearningCyberGradients.background(),
      ),
      child: Stack(
        children: const [
          Positioned(
          ...
          Positioned(
            ...
            child: _Blob(
              color:
                  LearningCyberColors.neonViolet,
              size: 320,
            ),
          ),
        ],
